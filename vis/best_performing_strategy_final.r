...
data <- read.csv(file.path(folder, '..', 'results', 'model_results', 'regional_annual_market_demand.csv'))

names(data)[names(data) == 'GID_0'] <- 'country'

data$scenario = factor(data$scenario, 
                       levels=c("low_50_25_5",
                                "baseline_50_25_5",
                                "high_50_25_5"),
                       labels=c("Low",
                                "Baseline",
...
data <- select(data, country, scenario, strategy, confidence, year,
...
demand = data[(
    data$strategy == '3G_umts_wireless_srn_baseline_low_low'
),]

demand = demand %>%
  group_by(country, scenario, strategy, confidence, year) %>%
  summarize(
    population_with_phones_f_over_10 = sum(population_with_phones_f_over_10),
    population_with_phones_m_over_10 = sum(population_with_phones_m_over_10),
    population_with_smartphones_f_over_10 = sum(population_with_smartphones_f_over_10),
    population_with_smartphones_m_over_10 = sum(population_with_smartphones_m_over_10),
  )

demand <- gather(demand, metric, value, population_with_phones_f_over_10:population_with_smartphones_m_over_10)

demand$metric = factor(demand$metric, 
                       levels=c("population_with_phones_f_over_10",
                                "population_with_phones_m_over_10",
                                "population_with_smartphones_f_over_10",
                                "population_with_smartphones_m_over_10"),
                       labels=c("Female cellphone adoption (>10 years)",
                                "Male cellphone adoption (>10 years)",
                                "Female smartphone adoption (>10 years)",
                                "Male smartphone adoption (>10 years)"))

demand_forecasts = ggplot(demand, aes(x=year, y=value/1e6, group=metric)) +
  ...
  scale_x_continuous(expand = c(0, 0), limits = c(2020,2030), breaks = seq(2020,2030,2)) +
  ...
  labs(title = "Cellphone and Smartphone Adoption Forecasts", 
       x = NULL, y = "Adoption (Millions)") +
  guides(colour=guide_legend(ncol=3), shape=guide_legend(ncol=3)) + 
  facet_grid(~scenario)

#export to folder
path = file.path(folder, 'figures_tables', 'demand_forecasts.png')
...
data <- read.csv(file.path(folder, '..', 'results', 'model_results', 'decile_mno_results_technology_options.csv'))
...
demand = data[(
  data$scenario == 'baseline_50_25_5' &
...
rm(data, demand, demand_densities, demand_supply, supply, supply_densities)

################################
#technology
data_tech <- read.csv(file.path(folder, '..', 'results', 'model_results', 'national_market_cost_results_technology_options.csv'))
data_tech <- select(data_tech, GID_0, scenario, strategy, confidence, societal_cost)
data_tech$societal_cost = round(data_tech$societal_cost,2)
...
#business model
data_bus_mod <- read.csv(file.path(folder, '..', 'results', 'model_results', 'national_market_cost_results_business_model_options.csv'))
...
#policy options
data_policy <- read.csv(file.path(folder, '..', 'results', 'model_results', 'national_market_cost_results_policy_options.csv'))
...
#Mixed options
data_mixed <- read.csv(file.path(folder, '..', 'results', 'model_results', 'national_market_cost_results_mixed_options.csv'))
...
rm(data_tech, data_bus_mod, data_policy, data_mixed, mixed, baseline, 
   passive, active, srn, spectrum_low, spectrum_high, tax_low, 
   tax_high)
...
results$scenario = factor(results$scenario, levels=c("low_50_25_5",
                                                     "baseline_50_25_5",
                                                     "high_50_25_5"),
                          labels=c("Low (<50 Mbps)",
                                   "Baseline (<50 Mbps)",
                                   "High (<50 Mbps)"))
...
rm(data_tech, data_bus_mod, data_policy, data_revenue, data_mixed,
...
results$tech_strategy = factor(results$tech_strategy,
                       levels=c('3G_umts_wireless', '3G_umts_fiber',
                                '4G_epc_wireless', '4G_epc_fiber',
                                '5G_nsa_wireless', '5G_nsa_fiber'),
                       labels=c('3G (W)', '3G (F)',
                                '4G (W)', '4G (F)', 
                                '5G (W)', '5G (F)'))
...
results_wide = results_wide %>%
  pivot_wider(names_from = Scenario, values_from = GMB)

# results_wide = select(results_wide, Scenario, Strategy, Metric, 'GMB')
...
table1 = results_wide %>%
  ...
  kable(escape = F, caption = 'Full Technology Results') %>%
  kable_classic("striped", full_width = F, html_font = "Cambria") %>%
  row_spec(0, align = "c") %>%
  add_header_above(
    c(" "= 2, "$Bn" = 3)) %>%
...
results_wide = results_wide[(results_wide$Strategy == '4G (W)'),]
...
table2 = results_wide %>%
  ...
  kable(escape = F, caption = 'Infrastructure Sharing Results') %>%
  kable_classic("striped", full_width = F, html_font = "Cambria") %>%
  row_spec(0, align = "c") %>%
  add_header_above(
    c(" "= 2, "$Bn" = 3)) %>%
  footnote(number = c("Technology Strategy: 4G with Wireless Backhaul.",
                      "Spectrum Pricing Strategy: Baseline.",
                      "Taxation Strategy: Baseline."))
...
table3 = results_wide %>%
  ...
  kable(escape = F, caption = 'Spectrum Pricing Results') %>%
  kable_classic("striped", full_width = F, html_font = "Cambria") %>%
  row_spec(0, align = "c") %>%
  add_header_above(
    c(" "= 2, "$Bn" = 3)) %>%
...
# 
# folder <- dirname(rstudioapi::getSourceEditorContext()$path)
# data <- read.csv(file.path(folder, '..', 'results', 'model_results', 'national_market_cost_results_technology_options.csv'))
# 
# data <- data[(data$strategy == "5G_nsa_wireless_baseline_baseline_baseline_baseline"),]
...
# ))
# 
# data$scenario = factor(data$scenario, levels=c("low_50_25_5",
#                                                      "baseline_50_25_5",
#                                                      "high_50_25_5"),
#                           labels=c("Low",
#                                    "Baseline",
...
###################
folder <- dirname(rstudioapi::getSourceEditorContext()$path)
data <- read.csv(file.path(folder, '..', 'results', 'model_results', 'national_market_cost_results_all_options.csv'))
...
baseline <- data %>%
  filter(str_detect(strategy, "4G_epc_wireless")) %>%
...
all_data$societal_cost = round(all_data$societal_cost / 1e9, 2)

all_data$scenario = factor(all_data$scenario, levels=c("low_50_25_5",
                                                       "baseline_50_25_5",
                                                       "high_50_25_5"),
                           labels=c("Low",
                                    "Baseline",
...
all_data = all_data %>%
  pivot_wider(names_from = scenario, values_from = societal_cost)

all_data$strategy = NULL

all_data = all_data[!duplicated(all_data[c('country', 'type')]),]
...
table6 = all_data %>%
  ...
  kable(escape = F, caption = '(A) Social Cost of Universal Access NPV 2020-2030') %>%
  kable_classic("striped", full_width = F, html_font = "Cambria") %>%
  row_spec(0, align = "c") %>%
  add_header_above(
    c(" "= 2, "$Bn" = 3)) %>%
...
###################
folder <- dirname(rstudioapi::getSourceEditorContext()$path)
data_revenue <- read.csv(file.path(folder, '..', 'results', 'model_results', 'national_mno_results_all_options.csv'))
...
data <- read.csv(file.path(folder, '..', 'results', 'model_results', 'decile_mno_cost_results_all_options.csv'))
data <- data[(data$confidence == 50),]
data <- data %>% filter(str_detect(strategy, "4G_epc_wireless"))
...
#get baseline rows
lowest <- filter(data, !strategy %in% "4G_epc_wireless_baseline_baseline_baseline_baseline")
...
all_data = all_data %>%
  pivot_wider(names_from = Scenario, values_from = decile)

# all_data <- spread(all_data, GMB, decile)
...
table7 = all_data %>%
  mutate(
    Low = cb_inverted(Low),
    Baseline = cb_inverted(Baseline),
    High = cb_inverted(High),
  ) %>%
  kable(escape = F, caption = '(B) Commercially Viable Population Coverage') %>%
  kable_classic("striped", full_width = F, html_font = "Cambria") %>%
  row_spec(0, align = "c") %>%
  footnote(number = c("Technology Strategy: 4G with Wireless Backhaul.",
                      "Infrastructure Sharing Strategy: Baseline.",
...
subsidy$scenario = factor(subsidy$scenario, levels=c("low_50_25_5",
                                                       "baseline_50_25_5",
                                                       "high_50_25_5"),
                           labels=c("Low",
                                    "Baseline",
...
table8 = subsidy %>%
  ...
  kable(escape = F,
        caption = '(C) Government Subsidy to Reach Universal Access NPV 2020-2030') %>%
  kable_classic("striped", full_width = F, html_font = "Cambria") %>%
  row_spec(0, align = "c") %>%
  add_header_above(
    c(" "= 1, "$Bn" = 3)) %>%
