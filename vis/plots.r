...
ggplot(data, aes(y=value, x=strategy_short, fill=Cost_Type)) + 
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, max_value+0.5)) +
...
ggplot(data, aes(y=value, x=strategy, fill=Cost_Type)) + 
  ...
  labs(title = "Social Cost of Universal Broadband by Infrastructure Sharing Strategy for The Gambia", 
       ...
       subtitle = "Reported for all scenarios and capacity per user targets",
       x = NULL, y = "Social Cost (Billions $USD)") +
  scale_y_continuous(expand = c(0, 0), limits = c(-min_value, max_value+0.1)) +
  theme(panel.spacing = unit(0.6, "lines")) +
  guides(fill=guide_legend(ncol=3, reverse = TRUE)) +
  facet_grid(scenario_capacity~scenario_adopt)
...
min_value = min(round(data$societal_cost/ 1e6))
max_value = max(round(data$societal_cost/ 1e6))
min_value[min_value > 0] = 0
...
data$value = round(data$value/1e6, 3)
...
ggplot(data, aes(y=value, x=strategy, fill=Cost_Type)) + 
  ...
  scale_fill_manual(values=c("#E1BE6A", "#40B0A6"), name=NULL) +
  theme(legend.position = "bottom") +
  labs(title = "Social Cost of Universal Broadband by Policy Options for The Gambia", 
       colour=NULL,
       subtitle = "Reported for all scenarios and capacity per user targets",
       x = NULL, y = "Social Cost (Millions $USD)") +
  scale_y_continuous(expand = c(0, 0), limits = c(-0, max_value+220)) +
  theme(panel.spacing = unit(0.6, "lines")) +
  guides(fill=guide_legend(ncol=3, reverse = TRUE)) +
  facet_grid(scenario_capacity~scenario_adopt)
