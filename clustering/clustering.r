...
### subset main variables
subset <- select(mydata, coverage_4g, pop_density, 
                gdp_per_cap, cellphone_penetration)
...
long <- select(data, country, coverage_4g, pop_density, 
               gdp_per_cap, cluster, income, region, ISO_3digit,
               cellphone_penetration)

#transform from wide to long
long <- gather(long, 'coverage_4g', 'pop_density', 
               'gdp_per_cap', 'cellphone_penetration', 
               key='metric', value='value')

#create factor levels/labels
long$metric = factor(long$metric, 
                     levels=c("gdp_per_cap", "pop_density", 
                              "coverage_4g", "cellphone_penetration"),
                    labels=c("GDP per capita", "Population Density", 
                             "4G Coverage", "Unique Cellphone Penetration"))
long$income = factor(long$income, 
...
boxplot <- ggplot(long, aes(cluster, value, colour=cluster, shape=Income)) + 
  ...
  scale_colour_manual(values = c("#F0E442","#E69F00","#D55E00", "#0072B2", 
                                 "#56B4E9","#009E73")) + 
  expand_limits(x = 0, y = 0) + 
  guides(colour=FALSE, shape=FALSE) + 
  scale_y_continuous(breaks = seq(-2, 8, by = 1), limits=c(-2,3)) +
  labs(title = "Summary Statistics by Cluster Number", 
       x='Cluster Number', y='Mean Centred Values',
...
#export data
write.csv(data, file.path(folder, '..', 'data', 'intermediate', 'data_clustering_results.csv'))
